...
import { KeyPair, EncryptedBox } from './types'
...
export function createBoxKeyPair(seed?: Buffer): KeyPair {
...
export function createBoxPublicFromSign(signKey: Buffer): Buffer {
  const boxKey = Buffer.allocUnsafe(BOX_PUBLICKEYBYTES)
  sodium.crypto_sign_ed25519_pk_to_curve25519(boxKey, signKey)
  return boxKey
}

export function createBoxKeyPairFromSign(signPair: KeyPair): KeyPair {
  const secretKey = sodium.sodium_malloc(BOX_SECRETKEYBYTES)
...
export function createSecretBoxKey(): Buffer {
  return secureRandomBytes(SECRETBOX_KEYBYTES)
}

export function createSecretBoxKeyFromPassword(
  password: Buffer,
  salt: Buffer,
...
export function encryptSecretBox(message: Buffer, key: Buffer): EncryptedBox {
